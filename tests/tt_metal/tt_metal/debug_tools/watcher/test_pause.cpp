...
void RunTest(MeshWatcherFixture* fixture, const std::shared_ptr<distributed::MeshDevice>& mesh_device) {
    ...
    if (is_quasar) {
        ...
        auto quasar_kernel_handle = tt::tt_metal::experimental::quasar::CreateKernel(
            ...
            tt::tt_metal::experimental::quasar::QuasarDataMovementConfig{.num_processors_per_cluster = num_dms});
    ...
    if (has_eth_cores) {
        ...
        erisc_kid =
            CreateKernel(program_, path, eth_core_ranges, tt_metal::EthernetConfig{.noc = tt_metal::NOC::NOC_0});
    ...
    if (has_ieth_cores) {
        ...
        SetCommonRuntimeArgs(program_, ierisc_kid, args);
    }
